...
const placeItems = () => {
    ...
    addItem('📦', randomInt(4, 8));
    addItem('🛢️', randomInt(3, 6));
...
const moveRobot = async instructions => {
    ...
    showMessage('🤖 El T1000 terminó su movimiento sin incidentes 🚷');
};
...
const checkCollision = () => {
    ...
    if (collisionItem) {
        ...
        if (['📦', '🛢️', '🚧', '🧱'].includes(collisionItem.emoji)) {
            showMessage('🫤 Lo sentimos, te has topado con un obstáculo y te han detectado; has perdido la misión ❌');
